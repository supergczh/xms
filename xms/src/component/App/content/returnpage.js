...
class ReturnPage extends Component {
    ...
    createPage(totalPage, currentPage) {
        ...
        pages.push(<Link onClick={this.prePageHandeler.bind(this)}
            to={{
                ...
                pathname: `/index/return/${currentPage - 1}`
            }}
            key={0}>
        ...
        if (totalPage <= 5) {
            ...
            pages.push(<Link key={totalPage}
                to={{
                    state: { id: totalPage },
                    pathname: `/index/return/${totalPage}`
                }}
    ...
    pageClick(currentPage) {
        let { findreturnkeyall, oddreturn, returnpagecount, findreturnpage,vue,onf,onf2,
            findreturnnumber, findreturnnumberpage, findreturnphoneall, findreturnphonepage,
            order,order2
        } = this.props
     console.log(onf,onf2,vue);
        console.log(currentPage);
     
        if (vue === '全部申请' && !onf && !onf2) {
        
            oddreturn(currentPage)
        } else if (vue === '待处理' && !onf && !onf2) {
            findreturnkeyall(0, currentPage)
            findreturnpage(0)
        } else if (vue === '已退货' && !onf && !onf2) {
            findreturnkeyall(1, currentPage)
            findreturnpage(1)
        }
        else if (vue === '已拒绝' && !onf && !onf2) {
            findreturnkeyall(2, currentPage)
            findreturnpage(2)
        }
        if (order && onf) {
...
}, (dispatch) => bindActionCreators(actionCreators, dispatch))(ReturnPage )

...
class RefurnPage extends Component {
    constructor(props) {
        super(props)
        this.state = {
            currentPage: 1, //当前页码
    ...
    createPage(totalPage, currentPage) {
        ...
        pages.push(<Link onClick={this.prePageHandeler.bind(this)}
            to={{
                ...
                pathname: `/index/refurn/${currentPage - 1}`
            }}
            key={0}>
        ...
        if (totalPage <= 5) {
            ...
            pages.push(<Link key={totalPage}
                to={{
                    state: { id: totalPage },
                    pathname: `/index/refurn/${totalPage}`
                }}
...
}, (dispatch) => bindActionCreators(actionCreators, dispatch))(RefurnPage)
